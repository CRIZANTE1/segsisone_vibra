...
class EmployeeManager:
    ...
    def load_data(self):
        try:
            ...
            company_columns = ['id', 'nome', 'cnpj']; employee_columns = ['id', 'nome', 'empresa_id', 'cargo', 'data_admissao']; aso_columns = ['id', 'funcionario_id', 'data_aso', 'vencimento', 'arquivo_id', 'riscos', 'cargo', 'tipo_aso']; training_columns = ['id', 'funcionario_id', 'data', 'vencimento', 'norma', 'modulo', 'status', 'arquivo_id', 'tipo_treinamento', 'carga_horaria']
    ...
    def initialize_sheets(self):
        try:
            from gdrive.config import ASO_SHEET_NAME, EMPLOYEE_SHEET_NAME, EMPLOYEE_DATA_SHEET_NAME, TRAINING_SHEET_NAME
            sheets_structure = {'EMPLOYEE_SHEET_NAME': ['id', 'nome', 'cnpj'], 'EMPLOYEE_DATA_SHEET_NAME': ['id', 'nome', 'empresa_id', 'cargo', 'data_admissao'], 'ASO_SHEET_NAME': ['id', 'funcionario_id', 'data_aso', 'vencimento', 'arquivo_id', 'riscos', 'cargo', 'tipo_aso'], 'TRAINING_SHEET_NAME': ['id', 'funcionario_id', 'data', 'vencimento', 'norma', 'modulo', 'status', 'arquivo_id', 'tipo_treinamento', 'carga_horaria']}
            for sheet_name, columns in sheets_structure.items():
                ...
                if not data: self.sheet_ops.criar_aba(sheet_name, columns)
                    ...
                    if sheet_name == ASO_SHEET_NAME and 'tipo_aso' not in header:
                         st.warning(f"A coluna 'tipo_aso' não foi encontrada na aba {ASO_SHEET_NAME} e será adicionada. Verifique sua planilha."); self.sheet_ops.limpar_e_recriar_aba(sheet_name, columns)
            return True
        except Exception as e:
            st.error(f"Erro ao inicializar as abas: {str(e)}"); return False
    def get_latest_aso_by_employee(self, employee_id):
        ...
        if not aso_docs.empty:
            ...
            if not aso_docs.empty:
                aso_docs['data_aso'] = aso_docs['data_aso'].dt.date; aso_docs['vencimento'] = pd.to_datetime(aso_docs['vencimento'], format='%d/%m/%Y', errors='coerce').dt.date
        return aso_docs
