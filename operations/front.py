...
# --- NOVAS FUNÇÕES DE CALLBACK PARA GERENCIAR ESTADO ---

def process_aso_pdf():
    """Analisa o ASO e guarda no session_state para confirmação."""
    if st.session_state.aso_uploader_tab:
        employee_manager = st.session_state.employee_manager
        with st.spinner("Analisando PDF do ASO..."):
            # Guarda o anexo e o ID do funcionário para usar depois
            ...
            # Analisa e guarda as informações extraídas
...
def process_training_pdf():
    """Analisa o Treinamento e guarda no session_state para confirmação."""
...
def front_page():
    ...
    with tab_dados:
        # ... (código da aba de dados permanece o mesmo) ...
        if selected_company:
            ...
            if not employees.empty:
                for index, employee in employees.iterrows():
                    ...
                    with st.expander(expander_title):
                        ...
                        if not latest_aso.empty: st.dataframe(latest_aso.style.apply(highlight_expired, axis=1), hide_index=True, use_container_width=True)
                        else: st.info("Nenhum ASO encontrado.")
                        st.markdown("##### Todos os Treinamentos")
                        if not all_trainings.empty: st.dataframe(all_trainings.style.apply(highlight_expired, axis=1), hide_index=True, use_container_width=True)
        ...
        if selected_company:
            if check_admin_permission():
                ...
                if not current_employees.empty:
                    ...
                    # Bloco de confirmação que SÓ aparece se a análise foi bem sucedida
                                ...
                                if st.button("Confirmar e Salvar ASO", type="primary"):
                                    ...
                                    if arquivo_id:
                                        employee_manager.add_aso(
                                            id=selected_employee_aso, arquivo_id=arquivo_id, **aso_info
                                        )
                                        st.success("ASO adicionado com sucesso!")
                                        # LIMPA O ESTADO para evitar re-envio
                                        del st.session_state.aso_info_para_salvar
                                        del st.session_state.aso_anexo_para_salvar
                                        del st.session_state.aso_funcionario_para_salvar
                                        st.rerun()
                        else:
                            st.error("Não foi possível extrair informações do PDF.")
                            del st.session_state.aso_info_para_salvar # Limpa estado em caso de falha
                            ...
                            with st.container(border=True):
                                ...
                                if st.button("Confirmar e Salvar Treinamento", type="primary"):
                                    ...
                                    if arquivo_id:
                                        ...
                                        del st.session_state.training_anexo_para_salvar
                                        del st.session_state.training_funcionario_para_salvar
                                        st.rerun()
                        else:
                            st.error("Não foi possível extrair informações do PDF.")
                            del st.session_state.training_info_para_salvar # Limpa estado em caso de falha
                else: st.warning("Cadastre funcionários nesta empresa primeiro.")
            else: st.error("Você não tem permissão para esta ação.")
        else: st.info("Selecione uma empresa na primeira aba.")
