...
class MatrixManager:
    def __init__(self, spreadsheet_id):
        ...
        # Load sheet names from config file
        with open("sheets_config.yaml", 'r', encoding='utf-8') as f:
            sheets_config = yaml.safe_load(f)
            self.function_sheet_name = sheets_config['funcoes']['name']
            self.training_matrix_sheet_name = sheets_config['matriz_treinamentos']['name']
    ...
    def _initialize_sheets(self):
        if not self.sheet_ops.carregar_dados_aba(self.function_sheet_name):
            self.sheet_ops.criar_aba(self.function_sheet_name, self.columns_functions)
        if not self.sheet_ops.carregar_dados_aba(self.training_matrix_sheet_name):
            self.sheet_ops.criar_aba(self.training_matrix_sheet_name, self.columns_matrix)

    def _load_functions_data(self):
        functions_data = self.sheet_ops.carregar_dados_aba(self.function_sheet_name)
    ...
    def _load_matrix_data(self):
        matrix_data = self.sheet_ops.carregar_dados_aba(self.training_matrix_sheet_name)
    ...
    def add_function(self, name, description):
        ...
        new_id = self.sheet_ops.adc_dados_aba(self.function_sheet_name, [name, description])
        if new_id:
            self._functions_df = None
    ...
    def add_training_to_function(self, function_id, required_norm):
        ...
        new_id = self.sheet_ops.adc_dados_aba(self.training_matrix_sheet_name, [str(function_id), required_norm])
        if new_id:
            self._matrix_df = None
    ...
    def save_extracted_matrix(self, extracted_data: list):
        ...
        if new_functions_to_add:
            self.sheet_ops.adc_dados_aba_em_lote(self.function_sheet_name, new_functions_to_add)
            self._functions_df = None
        ...
        if new_mappings_to_add:
            self.sheet_ops.adc_dados_aba_em_lote(self.training_matrix_sheet_name, new_mappings_to_add)
            self._matrix_df = None
        ...
        try:
            ...
            for norm in to_remove:
                ...
                if not mapping_to_delete.empty:
                    ...
                    if self.sheet_ops.excluir_dados_aba(self.training_matrix_sheet_name, mapping_id):
                        removed_count += 1
            self._matrix_df = None
        ...
        **Importante:** Responda APENAS com o bloco de código JSON. Se a função não exigir treinamentos, retorne `[]`.
        ...
        try:
            ...
            if not response_text:
                return None, "A IA não retornou uma resposta."
            try:
                match = re.search(r'```jsons*(\[.*\])s*```', response_text, re.DOTALL)
                if match:
